...
trait BotsPostsTable {
  ...
  class BotsPosts(_tableTag: Tag) extends profile.api.Table[BotsPostsRow](_tableTag, "bots_posts") {
    def * = (id, botId, postId) <> (BotsPostsRow.tupled, BotsPostsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(botId), Rep.Some(postId))).shaped.<>({r=>import r._; _1.map(_=> BotsPostsRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
