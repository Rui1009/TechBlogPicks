...
trait AccessTokensTable {
  ...
  implicit def GetResultAccessTokensRow(implicit
    e0: GR[String]
  ): GR[AccessTokensRow] = GR { prs =>
    import prs._
  ...
  class AccessTokens(_tableTag: Tag)
      extends profile.api.Table[AccessTokensRow](_tableTag, "access_tokens") {
    def * = (token, botId) <> (AccessTokensRow.tupled, AccessTokensRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(token), Rep.Some(botId))).shaped.<>(
      { r =>
        import r._; _1.map(_ => AccessTokensRow.tupled((_1.get, _2.get)))
      },
      (_: Any) =>
