      ...
      "invoke workSpaceRepository.find & workSpaceRepository.sendMessage once" in {
        forAll(workSpaceGen, botIdGen, botGen, channelTypedChannelMessageGen) {
          (workSpace, botId, bot, channel) =>
            ...
            val returnedWorkSpace = workSpace.copy(channels = Seq(channel.copy(id = channel.id, history = Seq())), bots = Seq(bot.copy(id = Some(botId))))
            val targetChannel = returnedWorkSpace.findChannel(params.channelId).unsafeGet

            when(workSpaceRepo.find(params.workSpaceId)).thenReturn(
              Future.successful(Some(returnedWorkSpace))
            )
            val onboardingMessage = returnedWorkSpace.botCreateOnboardingMessage(params.botId).unsafeGet
            val targetBot = returnedWorkSpace.botPostMessage(params.botId, targetChannel.id, onboardingMessage).unsafeGet
            when(workSpaceRepo.sendMessage(targetBot, targetChannel, onboardingMessage)).thenReturn(Future.successful(()))
            ...
            verify(workSpaceRepo).sendMessage(targetBot, targetChannel, onboardingMessage)
    ...
    "succeed when any channel message exists" should {
      "invoke workSpaceRepository.find & workSpaceRepository.sendMessage never invokes" in {
        forAll(workSpaceGen, botIdGen, botGen, channelTypedChannelMessageGen) {
          (workSpace, botId, bot, channel) =>
            val params = Params(botId, workSpace.id, channel.id)
            val returnedWorkSpace = workSpace.copy(channels = Seq(channel.copy(id = channel.id)), bots = Seq(bot.copy(id = Some(botId))))
            val targetChannel = returnedWorkSpace.findChannel(params.channelId).unsafeGet
      ...
      "throw use case error" in {
        forAll(workSpaceGen, botIdGen, channelTypedChannelMessageGen) {
          (workSpace, botId, channel) =>
            ...
            val result = new PostOnboardingMessageUseCaseImpl(workSpaceRepo)
              .exec(params)
