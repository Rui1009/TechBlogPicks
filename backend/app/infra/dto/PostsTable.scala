...
trait PostsTable {
  ...
  case class PostsRow(id: Long, url: String, title: String, author: String, postedAt: Long, createdAt: Long, testimonial: Option[String] = None)
  /** GetResult implicit for fetching PostsRow objects using plain SQL queries */
  implicit def GetResultPostsRow(implicit e0: GR[Long], e1: GR[String], e2: GR[Option[String]]): GR[PostsRow] = GR{
    prs => import prs._
    PostsRow.tupled((<<[Long], <<[String], <<[String], <<[String], <<[Long], <<[Long], <<?[String]))
  }
  ...
  class Posts(_tableTag: Tag) extends profile.api.Table[PostsRow](_tableTag, "posts") {
    def * = (id, url, title, author, postedAt, createdAt, testimonial) <> (PostsRow.tupled, PostsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(url), Rep.Some(title), Rep.Some(author), Rep.Some(postedAt), Rep.Some(createdAt), testimonial)).shaped.<>({r=>import r._; _1.map(_=> PostsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    val testimonial: Rep[Option[String]] = column[Option[String]]("testimonial", O.Default(None))
  }
