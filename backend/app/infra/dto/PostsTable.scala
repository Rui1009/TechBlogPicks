...
trait PostsTable {
  ...
  case class PostsRow(id: Long, url: String, title: String, author: String, postedAt: Long, createdAt: Long)
  ...
  class Posts(_tableTag: Tag) extends profile.api.Table[PostsRow](_tableTag, "posts") {
    ...
    def ? = ((Rep.Some(id), Rep.Some(url), Rep.Some(title), Rep.Some(author), Rep.Some(postedAt), Rep.Some(createdAt))).shaped.<>({r=>import r._; _1.map(_=> PostsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
