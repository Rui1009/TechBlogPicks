...
trait WorkSpacesTable {
  ...
  case class WorkSpacesRow(token: String, botId: String, teamId: String = "hoge")
  /** GetResult implicit for fetching WorkSpacesRow objects using plain SQL queries */
  implicit def GetResultWorkSpacesRow(implicit e0: GR[String]): GR[WorkSpacesRow] = GR{
    prs => import prs._
  ...
  class WorkSpaces(_tableTag: Tag) extends profile.api.Table[WorkSpacesRow](_tableTag, "work_spaces") {
    def * = (token, botId, teamId) <> (WorkSpacesRow.tupled, WorkSpacesRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(token), Rep.Some(botId), Rep.Some(teamId))).shaped.<>({r=>import r._; _1.map(_=> WorkSpacesRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
